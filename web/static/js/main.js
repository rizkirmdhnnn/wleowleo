document.addEventListener('DOMContentLoaded', function() {
    ...
    socket.onmessage = function(event) {
        ...
        switch(data.type) {
            case 'scraper_log':
                addScraperLog(data.title || 'Scraper', data.message, data.status);
                break;
            case 'downloader_log':
    ...
    function updateConfig() {
        const config = {
            base_url: document.getElementById('base-url') ? document.getElementById('base-url').value : '',
            from_pages: parseInt(document.getElementById('from-page').value),
            to_pages: parseInt(document.getElementById('to-page').value),
            limit_concurrent_download: parseInt(document.getElementById('concurrent-download').value)
        };
    ...
    function animateCounter(element, newValue) {
        ...
        const step = Math.max(1, Math.floor(Math.abs(newValue - currentValue) / 20));
    ...
    function addScraperLog(title, message, status) {
        ...
        const borderColor = status === 'error' ? 'border-l-error' : 'border-l-success';
    ...
    function addDownloaderLog(message, progress, total) {
        const logItem = document.createElement('div');
        logItem.className = 'p-3 mb-2 rounded-md bg-bg-dark border-l-4 border-l-transparent transition-transform duration-200 hover:translate-x-1 w-full';
        ...
        logItem.innerHTML = `
            ...
            <p class="text-xs text-right mt-1">${progress}/${total} Part (${percent}%)</p>
        `;
